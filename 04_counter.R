...
# In this script we import the data created with "03_observation_counter.R", 
# we merge them and we finally select the tickers that have at least 100 and 
# an average of 480 observation per day.
...
filename <- list.files(pattern = ".*csv$")[grep("_count",list.files(pattern = ".*csv$"))]
...
ggplot(data =  min_obs[,-1], aes(x = Ticker, y = Counter)) + geom_col() +theme(legend.position="none") + theme(axis.text.x=element_blank())
...
ggplot(data =  selection, aes(x = Ticker, y = Counter)) + geom_col() + #theme_few()+
  geom_text(data=selection, aes(x = Ticker, y = Counter, label = Ticker),vjust=0.5, hjust=-0.25, angle = 90, size = 14/5*0.85) +
